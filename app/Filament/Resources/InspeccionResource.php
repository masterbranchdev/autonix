...
class InspeccionResource extends Resource
{
        ...
        return $form
            ->schema([
                ...
                \Filament\Forms\Components\Select::make('orden_servicio_id')
                    ...
                    ->getSearchResultsUsing(function (string $search) {
                        ...
                        return \App\Models\OrdenServicio::with('vehiculo.cliente')
                            ...
                            ->where(function ($query) use ($cleanSearch) {
                                $query->where('folio', 'ilike', "%{$cleanSearch}%")
                    ...
                    ->preload()
                    ->required()
                    ->unique(table: 'inspecciones', column: 'orden_servicio_id', ignoreRecord: true)
                    ->validationMessages([
                        'unique' => 'Bloqueo: Esta orden de servicio ya cuenta con una inspección registrada.',
                    ])
                    ->columnSpanFull(),
                ...
                \Filament\Forms\Components\Tabs::make('Inspeccion Completa')
                    ->tabs([
                        ...
                        \Filament\Forms\Components\Tabs\Tab::make('Llantas y Balatas')
                            ...
                            ->schema([
                                \Filament\Forms\Components\Section::make('Presión y Vida de Llantas')
                                    ->schema([
                                ...
                                \Filament\Forms\Components\Section::make('Grosor de Balatas (Frenos)')
                                    ->schema([
                                ...
                                \Filament\Forms\Components\Section::make('Servicios Adicionales Recomendados')
                                    ->schema([
                                        ...
                                        \Filament\Forms\Components\Grid::make(3)->schema([
                                            \Filament\Forms\Components\TextInput::make('adicionales.fugas_especificar')->label('Especificar Fugas de aceite/fluidos'),
    ...
    private static function makeLlantaForm($key, $label)
    {
        return \Filament\Forms\Components\Fieldset::make($label)
            ->schema([
                \Filament\Forms\Components\TextInput::make("llantas.{$key}.psi")
                    ->label('PSI')
                    ->numeric(),
                \Filament\Forms\Components\TextInput::make("llantas.{$key}.mm")
                    ->label('Prof. (mm)')
                    ->numeric(),
                \Filament\Forms\Components\Select::make("llantas.{$key}.estado")
                    ->label('Estado')
                    ->options([
                        'verde' => 'OK (>50%)',
                        'amarillo' => 'Regular (20-50%)',
                        'rojo' => 'Reemplazar (<20%)'
                    ])
                    ->native(false), // Le da un diseño desplegable premium
            ])
            ->columns(2)
            ->columnSpan(1);
    }
    ...
    private static function makeBalataForm($key, $label)
    {
        return \Filament\Forms\Components\Fieldset::make($label)
            ->schema([
                \Filament\Forms\Components\TextInput::make("balatas.{$key}.mm")
                    ->label('Grosor (mm)')
                    ->numeric(),
                \Filament\Forms\Components\Select::make("balatas.{$key}.estado")
                    ->label('Estado')
                    ->options([
                        'verde' => 'OK (>50%)',
                        'amarillo' => 'Regular (20-50%)',
